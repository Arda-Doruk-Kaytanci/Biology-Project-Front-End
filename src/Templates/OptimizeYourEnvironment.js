...
const OptimizationSystem = () => {
  ...
  function recommendPlants(weatherData, balconyArea) {
    ...
    const suitablePlants = plantDatabase.filter(
      (plant) =>
        ...
        (inputs.light === "Çok"
          ...
          : inputs.light === "Az"
          ? false
          : inputs.light === "Orta" && plant.lightRequired === "Orta Derecede Işık Almalı"
          ? true
          : false)
    );
    ...
    return suitablePlants.map((plant) => ({
      name: plant.name,
      src: plant.src,
      amount: (balconyArea - 5 ) / plant.spaceRequired
    }));
  }
  useEffect(() => {
    const time = new Date();
    ...
    <div className="optimize">
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-container">
          ...
          <label>
            <p>Balkonunuz Ne Kadar Güneş Alıyor?</p>
          </label>
          <select className="optimize-select" onChange={handleInputChange} name="light">
            <option value="Az">Hiç Güneş Almıyor</option>
            <option value="Orta">Orta Derecede Güneş Alıyor</option>
            <option value="Çok">Çok Güneş Alıyor</option>
          </select>
        ...
        <div className="response-container">
          ...
          <div className="crops-container">
            {recommendPlants(
              ...
              return (
                <div key={index}>
                  <CropLink head="Main" amount={item.amount} name={item.name} img={item.src} />
                </div>
              );
            })}
          </div>
